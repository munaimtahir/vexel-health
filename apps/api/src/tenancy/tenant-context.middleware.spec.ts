...
describe('TenantContextMiddleware', () => {
    ...
    afterEach(() => {
        jest.clearAllMocks();
        process.env.TENANCY_DEV_HEADER = '0'; // Default safe
    });
    ...
    it('should resolve tenant from hostname (production path)', async () => {
        ...
        (prisma.tenantDomain.findUnique as jest.Mock).mockResolvedValue({ tenantId: 'tenant-123' });
    ...
    it('should use header if localhost + env enabled (dev path)', async () => {
        process.env.TENANCY_DEV_HEADER = '1';
        const req: any = {
    ...
    it('should NOT use header if env disabled even on localhost', async () => {
        process.env.TENANCY_DEV_HEADER = '0';
        const req: any = {
            headers: { host: 'localhost:3000', 'x-tenant-id': 'dev-tenant-id' }
        };
        const res: any = {};
